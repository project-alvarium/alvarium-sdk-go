...
func readPrivateKey(cfg config.HederaConfig) (hedera.PrivateKey, error) {
	...
	// Reading key directly from bytes using hedera.PrivateKeyFromBytesDer()
	// requires some additional encoding overhead thats already done by
	// hedera.PrivateKeyFromStringDer()
	privateKey, err := hedera.PrivateKeyFromStringDer(string(b))
	if err != nil {
		return hedera.PrivateKey{}, err
	}
